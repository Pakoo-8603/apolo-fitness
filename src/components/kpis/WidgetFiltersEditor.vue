<template>
  <div class="space-y-3">
    <div class="flex items-center justify-between">
      <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-200">Filtros del widget</h3>
      <button type="button" class="text-sm text-primary-600 hover:underline" @click="addFilter">Agregar filtro</button>
    </div>
    <div v-if="!localFilters.length" class="rounded border border-dashed border-slate-300 p-4 text-sm text-slate-500">Sin filtros adicionales.</div>
    <div v-for="filter in localFilters" :key="filter._localKey" class="rounded-lg border border-slate-200 bg-white/80 p-3 shadow-sm dark:border-slate-700 dark:bg-slate-900/60">
      <div class="flex flex-wrap gap-3">
        <label class="flex flex-col text-xs font-medium text-slate-600 dark:text-slate-300">
          Alias
          <select v-model="filter.target_alias" class="mt-1 rounded border border-slate-300 bg-white px-2 py-1 text-sm dark:border-slate-600 dark:bg-slate-800">
            <option value="">Métrica principal</option>
...
function syncFromProps () {
  ...
  for (const filter of props.modelValue) {
    localFilters.push({
      ...
      target_alias: filter.target_alias || '',
      _localKey: `widget-filter-${seq++}`,
